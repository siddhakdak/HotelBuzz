...
const UpdateRoom = () => {
  ...
  useEffect(() => {
    axios.get(`https://scaler-sde-assignment.vercel.app/booking/${id}`)
      .then((res) => {
        setData(res.data);
        setTip(res.data.tip);
  ...
  const handleSubmit = async (e) => {
   ...
   const result = await confirm(`Are you sure you want to update this booking? The total bill is ${totalBill}`, {
    ...
    axios.put(`https://scaler-sde-assignment.vercel.app/update-booking/${id}`, updateRoom);
    window.location = '/';
  };
  ...
  useEffect(() => {
    axios
      .get(`https://scaler-sde-assignment.vercel.app/all-bookings/`)
      .then((res) => {
        setAllData(res.data);
